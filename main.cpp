...
/*struct MalformedHTML {};
struct Parsed {};

template<int POSITION> struct Err 
{ 
  Err() {Parsed p = MalformedHTML();}
};

template<> struct Err<0> 
{ 
  Err(){}
};
*/
...
They index into a std::array of fixed size
*/
...
int main()
{
  ...
  for(int i = 0; i < 5; i++)
  {
    ...
    if(node.sibling > -1) cout << "sibling:" <<  node.sibling << endl;
    if(node.child > -1) cout << "child:" << node.child << endl;
